...
the cost funtion is bpr with regularizer
'''
...
sample_num = 10
Tf_constant_1 = tf.constant(1, dtype=tf.float32)
Tf_constant_0 = tf.constant(0, dtype=tf.float32)
def get_R(X):
    Y, alpha = X[0], X[1]
...
def add_regularizer(X):
    ...
    #batched_num = tf.nn.l2_loss(X2 * Tf_constant_0 + Tf_constant_1) / 128.0
    ans = tf.reduce_sum(tf.abs(output1))# + 0.1 * (tf.nn.l2_loss(X1) + 11.0 * tf.nn.l2_loss(X2))
    return ans #/ batched_num
...
def bpr(item_indexMax, user_indexMax, inputDim, BPRlen = sample_num + 1):
    ...
    ItemEmbedding = Embedding(item_indexMax, inputDim, name = "ItemEmbedding")
    ...
    #output2 = Flatten(name = "flat_alpha2")(merged2)   
...
def runModelBPR(model, userRecords_index, item_indexMax, user_indexMax, batch_num = 200):
    ...
    for i in range(batch_num): #for the batch of the sequences
        ...
        if trainX.shape[0] > 0:
            #model.fit([trainX, trainY], train_label, batch_size = 50, nb_epoch=50, callbacks=[keras.callbacks.EarlyStopping(monitor='loss', patience=1)], verbose=2)
	    #model.fit([trainX, trainY], train_label, batch_size = 50, nb_epoch=50, verbose=2, shuffle=True)
	    model.train_on_batch([trainX, trainY], train_label)
    #return error 
...
def saveModelByBatch(model, userRecords_index, item_indexMax, user_indexMax, index2item, index2user):   
    ...
    saveEmbedding(getUserEmbedding, user_seq, 1, index2user, "user.BPROutput")
    saveEmbedding(getItemEmbedding, item_seq, sample_num + 1, index2item, "item.BPROutput")
