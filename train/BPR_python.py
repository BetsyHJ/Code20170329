...
for simple, we use classification to replace the bpr. Every time, we want to make the true result 1.
'''
...
def runModelBPR(model, userRecords_index, item_indexMax, user_indexMax, batch_num = 100):
    ## batch_num reset
...
def saveEmbedding(getEmbedding, sequences, Maxlen, index2id, filename):
    ...
    for i in range(batch_num): #for the batch of the sequences
        ...
        seq = np.array([sequences_batch], dtype = 'int32')
